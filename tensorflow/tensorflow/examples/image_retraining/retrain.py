...
def create_image_lists(image_dir, test_dir, testing_percentage, validation_percentage):
  ...
  is_root_dir = True
  for (sub_dir, sub_dir_test) in zip(sub_dirs, sub_dirs_test):
    #print(sub_dir)
    #print(sub_dir_test)
    if is_root_dir:
      is_root_dir = False
    ...
    dir_name_test = os.path.basename(sub_dir_test)
    if (dir_name, dir_name_test) == zip(image_dir, test_dir):
      continue
    tf.logging.info("Looking for images in '" + dir_name + "' and '" + dir_name_test + "'")
    for extension in extensions:
      file_glob = os.path.join(image_dir, dir_name, '*.' + extension)
      file_list.extend(gfile.Glob(file_glob))
      file_glob_test = os.path.join(test_dir, dir_name_test, '*.' + extension)
      file_list_test.extend(gfile.Glob(file_glob_test))
    if not file_list or not file_list_test:
...
def get_image_path(image_lists, label_name, index, bottleneck_dir, category):
...
def get_bottleneck_path(image_lists, label_name, index, bottleneck_dir,
  ...
  return get_image_path(image_lists, label_name, index, bottleneck_dir,
                        category) + '_' + category + '_' + architecture + '.txt'
...
def create_bottleneck_file(bottleneck_path, image_lists, label_name, index,
                           image_dir, image_dir_test, category, sess, jpeg_data_tensor,
                           decoded_image_tensor, resized_input_tensor,
                           bottleneck_tensor):
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir, image_dir_test,
...
def cache_bottlenecks(sess, image_lists, image_dir, image_dir_test, bottleneck_dir,
    ...
    for category in ['training', 'testing', 'validation']:
      ...
      for index, unused_base_name in enumerate(category_list):
        get_or_create_bottleneck(
            sess, image_lists, label_name, index, image_dir, image_dir_test, category,
  ...
  if how_many >= 0:
    ...
    for unused_i in range(how_many):
      ...
      bottleneck = get_or_create_bottleneck(
          sess, image_lists, label_name, image_index, image_dir, image_dir_test, category,
          bottleneck_dir, jpeg_data_tensor, decoded_image_tensor,
          resized_input_tensor, bottleneck_tensor, architecture)
...
def get_random_distorted_bottlenecks(
    sess, image_lists, how_many, category, image_dir, image_dir_test, input_jpeg_tensor,
...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  with tf.name_scope(layer_name):
    ...
    with tf.name_scope('Wx_plus_b'):
      if FLAGS.twin:
        logits = tf.concat([bottleneck_input_l, bottleneck_input_r], 1)
      else:
...
def add_evaluation_step(result_tensor, ground_truth_tensor):
  ...
  return evaluation_step, prediction, result
...
def create_model_info(architecture):
  ...
  if FLAGS.twin:
      ...
      model_file_name = '0.1_240000_output_graph.pb'
      input_mean = 128
      input_std = 128
...
def main(_):
  ...
  image_lists = create_image_lists(FLAGS.image_dir, FLAGS.image_dir_test, FLAGS.testing_percentage,
  ...
  with tf.Session(graph=graph) as sess:
    ...
    evaluation_step, prediction, result = add_evaluation_step(
      ...
      if FLAGS.twin:
        ...
        for train_ground_truth_l, train_ground_truth_r in zip(train_ground_truths_l, train_ground_truths_r):
          if np.argmax(train_ground_truth_l) == np.argmax(train_ground_truth_r):
            train_ground_truth.append([1.0, 0.0])
          else:
            train_ground_truth.append([0.0, 1.0])

        train_ground_truth = np.reshape(train_ground_truth, (-1, 2))
      else:
        (train_bottlenecks,
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        if FLAGS.twin:
          train_accuracy, cross_entropy_value = sess.run(
            [evaluation_step, cross_entropy], feed_dict={bottleneck_input_l: train_bottlenecks_l,
                                      bottleneck_input_r: train_bottlenecks_r, ground_truth_input:train_ground_truth})
        else:
          train_accuracy, cross_entropy_value = sess.run(
        ...
        if FLAGS.twin:
          validation_summary, validation_accuracy = sess.run(
            ...
            feed_dict={bottleneck_input_l: validation_bottlenecks_l, bottleneck_input_r: validation_bottlenecks_r,
    ...
    if FLAGS.twin:
      test_bottlenecks_l, test_ground_truths_l, test_filenames_l = (
        get_random_cached_bottlenecks(
      ...
      k = 0
      for test_im_r, ground_r in zip(test_bottlenecks_l, test_ground_truths_l):
          k += 1
          test_r = []
          test_labels_r = []
          for i in range(0, len(test_bottlenecks_l)-k):
              #print(i)
              test_r.append(test_im_r)
              test_labels_r.append(ground_r)
          test_ground_truth = []
          for test_label_l, test_label_r in zip(test_ground_truths_l[k:], test_labels_r):
              if np.argmax(test_label_l) == np.argmax(test_label_r):
                  test_ground_truth.append([1.0, 0.0])
              else:
                  test_ground_truth.append([0.0, 1.0])
          test_ground_truth = np.reshape(test_ground_truth, (-1, 2))
          if k < len(test_ground_truths_l):
              test_accuracy, predictions, results = sess.run(
                  [evaluation_step, prediction, result],
                  feed_dict={bottleneck_input_l: test_bottlenecks_l[k:], bottleneck_input_r: test_r,
                             ground_truth_input: test_ground_truth})
              final_test_accuracy += test_accuracy * 100
              # for truth, pred in zip(test_ground_truth, predictions):
              #   #tf.logging.info('{} {}'.format(predictions.shape, results.shape))
              #   tf.logging.info('ground truth: {} prediction: {}'.format(truth, pred))
              tf.logging.info('%d test accuracy = %.1f%% (N=%d)' %
                              (k, test_accuracy * 100, len(test_r)))
